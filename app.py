...
[data-testid="stTabs"] [aria-selected="true"] { color: #ffd95a !important; border-bottom-color: #ffd95a !important; }
    ...
    with manager_tab:
        st.caption("A combined view of the latest Goal Management and Business Essentials measures."); agency_logo = __import__("pathlib").Path(__file__).resolve().parent / "assets" / "agency-logo.jpg"; st.markdown(f'<div style="width:100%;text-align:right;line-height:0;margin:0 0 8px 0"><img src="data:image/jpeg;base64,{__import__("base64").b64encode(agency_logo.read_bytes()).decode()}" alt="Grace Harbour Creator Network" style="width:150px;max-width:20vw;height:auto"></div>', unsafe_allow_html=True) if agency_logo.exists() else None; dashboard_manager_choices = ["Agency", *[m for m in sorted(creators.get("_manager", pd.Series(dtype=str)).dropna().astype(str).unique().tolist()) if m.strip() and m.strip() != "-"]]; dashboard_choice = st.selectbox("Dashboard view", dashboard_manager_choices, key="dashboard_manager_filter")
        if creators.empty:
