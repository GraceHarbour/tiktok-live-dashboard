...
def _wheel_replay_html(prize_name: str, candidate_names: list[str], winners: list[str]) -> str:
    ...
    .spin-button{{margin:14px auto 8px;padding:14px 30px;border:0;border-radius:999px;background:linear-gradient(135deg,#ffd34d,#ff9f1c);color:#071425;font-size:20px;font-weight:900;cursor:pointer;box-shadow:0 0 20px rgba(255,211,77,.55)}} .spin-button:disabled{{opacity:.55;cursor:not-allowed}}
    </style></head><body><h1>{safe_prize}</h1><div class="subtitle">Monthly Creator Prize Drawing</div><div class="stage"><div class="pointer"></div><canvas id="wheel" width="720" height="720"></canvas><div class="center"><div class="center-prize">{safe_prize}</div><div id="name">Ready</div></div></div><button id="spinButton" class="spin-button">Spin Wheel</button><div id="status">Press Spin Wheel when you are ready.</div><div id="results" class="results"></div><script>
    const candidates={names_json}, winners={winners_json}, colors=['#ff2d95','#18bfff','#7b4dff','#ff9f1c','#0ad5a8','#ff4d4d','#4169e1','#d83cff'];
    const wheel=document.getElementById('wheel'),ctx=wheel.getContext('2d'),nameBox=document.getElementById('name'),status=document.getElementById('status'),results=document.getElementById('results'),spinButton=document.getElementById('spinButton');
    ...
    function updateSpinButton(){{if(spin>=winners.length){{spinButton.disabled=true;spinButton.textContent='Drawing Complete';status.textContent='Congratulations to all winners of {safe_prize}!';return;}}spinButton.disabled=false;spinButton.textContent='Spin for Winner '+(spin+1);}}
    function runSpin(){{if(spin>=winners.length)return;spinButton.disabled=true;spinButton.textContent='Spinning…';const winnerNumber=spin+1;status.textContent='Spinning for Winner '+winnerNumber+' of '+winners.length+' — {safe_prize}';rotation+=2160+Math.floor(Math.random()*360);wheel.style.transform='rotate('+rotation+'deg)';let ticks=0;const timer=setInterval(()=>{{nameBox.textContent=candidates[Math.floor(Math.random()*candidates.length)]||'Spinning';if(++ticks>=38){{clearInterval(timer);const winner=winners[spin];nameBox.textContent=winner;status.textContent='Congratulations to Winner '+winnerNumber+': '+winner+' — {safe_prize}';addWinner(winnerNumber,winner);spin++;updateSpinButton();}}}},110);}}
    if(!winners.length){{spinButton.disabled=true;status.textContent='Wheel ready — create a drawing to select winners.';}}else{{spinButton.addEventListener('click',runSpin);updateSpinButton();}}
    </script></body></html>"""
