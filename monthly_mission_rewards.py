...
def _wheel_replay_html(prize_name: str, candidate_names: list[str], winners: list[str]) -> str:
    ...
    if(!winners.length){{spinButton.disabled=true;status.textContent='Wheel ready — create a drawing to select winners.';}}else{{spinButton.addEventListener('click',()=>{{spinButton.disabled=true;spinButton.textContent='Spinning…';runSpin();}},{{once:true}});}}
    </script></body></html>"""
